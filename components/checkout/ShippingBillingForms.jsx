...
export function ShippingDetails({countries, options,shippingCountry, setShippingCountry, shippingSubdivision, setShippingSubdivision,subdivisions}){
...
export function BillingDetails() {
  ...
  return (
    <div className={styles.customerInfoContainer}>
      ...
      <label>Email<input required name='billingEmail' type='text' placeholder='abc123@example.com' value={email} onChange={(e) => setEmail(e.target.value)} /></label>
      <label>Name<input required name='billingName' type='text' placeholder='John Doe' value={name} onChange={(e) => setName(e.target.value)} /></label>
      <label>Country<input required name='billingCountry' type='text' placeholder='United States' value={country} onChange={(e) => setCountry(e.target.value)} /></label>
      <label>State<input required name='billingState' type='text' placeholder='New Jersey' value={subdivision} onChange={(e) => setSubdivision(e.target.value)} /></label>
      <label>City<input required name='billingCity' type='text' placeholder='Trenton' value={city} onChange={(e) => setCity(e.target.value)} /></label>
      <label>Postal Code<input required name='billingZip' type='text' placeholder="08608" value={postalCode} onChange={(e) => setPostalCode(e.target.value)}/></label>
      <label>Address<input required name='billingLine1' type='text' placeholder='20 S Montgomery St' value={billingLine1} onChange={(e) => setBillingLine1(e.target.value)} /></label>
      <label>Apartment, Suite, etc.. (Optional)<input name='billingLine2' type='text' placeholder='Apt. 1' value={billingLine2} onChange={(e) => setBillingLine2(e.target.value)}/></label>
    </div>
  )
}